...
description: "Test quality review sub-agent for toolkit-pr-review. Covers RUST-TEST-001 and 10 test anti-patterns. Returns JSON array only."
tools: Read, Bash
model: inherit
---
...
- 10 common test anti-patterns that make tests weak or misleading
- Tests that don't actually verify behavior
- Production code added or changed with no tests for it at all
...
3. `/tmp/toolkit-pr-review-$REVIEW_ID/files/<escaped-path>` — full source file contents. For a file listed in `context.json`'s `deleted_files`, this is the file's **pre-deletion** content from the base commit (the file no longer exists at the review head) — use it to see what test code was removed.
...
- A function added or changed with no tests is a finding **where the risk justifies it**. Do not require exhaustive testing for a trivial refactor or a rename — that turns this rule into a false-positive engine on refactor PRs.
...
- **A regression test must exercise the failure path the PR claims to fix.** When the change says it fixes a specific bug, check that a new test reproduces *that* path. A test covering an adjacent surface — the same subsystem in the same degraded mode, but not the operation that actually broke — leaves the bug unguarded and is one of the highest-value findings here.
- **A new tunable, threshold or interval needs a test that pins the behavior it guards.** A constant like a poll interval or a retry floor is what keeps a loop from degenerating; without a test, changing it silently changes runtime behavior and nothing fails.
...
You run whenever the diff contains Rust, whether or not it contains any tests. That is deliberate:
a PR that adds production code and no tests at all is the main thing `RUST-TEST-001` exists to
catch, and it is invisible if you only look at test code.

Two different scopes apply:

- **`RUST-TEST-001`** — look at the **production** code added or changed in the diff and ask whether
  tests exist for it. Anchor the finding on the added or changed production line itself (a new
  function's signature line is the natural choice); that line is in `changed_ranges`, so it is a
  normal line-anchored comment. Absence of a test file is not a reason to skip the finding.
- **`TEST-QUALITY-1` through `TEST-QUALITY-10`** — look at test code visible in the diff. If the
  diff has no test code, these produce nothing, which is correct.

- Focus the `TEST-QUALITY-*` checks on test functions and test modules visible in the diff (added or modified).
- Test indicators:
  - `#[test]` functions
