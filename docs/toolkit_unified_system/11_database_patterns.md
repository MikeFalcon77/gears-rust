...
- **`join_cte` is an inner join**, so an outer row repeats once per matching CTE row. Use
  `.distinct()` when the CTE is a membership set rather than a 1:1 join.
- **Recursive walks need a depth cap**, passed to `RecursiveCte::new`. PostgreSQL's `CYCLE`
  ...
  termination guarantee. There is no visited set, so the walk enumerates *paths*, not
  distinct nodes — it suits bounded, sparse, acyclic hierarchies. For a general (cyclic or
  hub-heavy) graph, expand one hop per query with `cte()` and keep the frontier in the
  caller, where you control deduplication between hops.
...
- [ ] Use raw SQL only in `migrations/*.rs` — including raw `WITH`.
