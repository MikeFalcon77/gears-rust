...
| **(12) Profile 2 (Self-Hosted) is undesigned** (§3.15, §3.16). Three gaps: transport (UDS for single-host), endpoint resolution (none exists today), and topology — process count is no longer a correctness question (any process serves any lease, §3.19) but *backend scope* still is: one process per host is safe only if every host's process shares one backend configuration | Platform + cluster owner | Design P2 explicitly; make the shared-backend requirement checkable at startup |
