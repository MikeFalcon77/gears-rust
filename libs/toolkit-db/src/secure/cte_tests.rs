...
fn recursive_cte_depth_cap_reflects_the_requested_bound() {
    ...
    for backend in BACKENDS {
        let three = recursive_sql(backend, 3);
        let ninety = recursive_sql(backend, 90);
        assert_ne!(
            three, ninety,
            "{backend:?} produced identical SQL for different depth caps"
        );
    }
}
...
#[test]
fn order_by_can_target_a_cte_column() {
    ...
    for backend in BACKENDS {
        let sql = scoped_nodes(&scope)
            ...
            .order_by(
                (
                    Alias::new("subtree"),
                    Alias::new(RecursiveCte::<node::Entity>::DEPTH_COLUMN),
                ),
                Order::Asc,
            )
            .build_statement(backend)
        ...
        assert!(
            ...
            "{backend:?} did not emit ORDER BY: {sql}"
        );
        // The ORDER BY must reference the CTE's depth column, not the entity's.
