...
Agent definitions live in `docs/pr-review/agents/` and carry their own rules. The shared
conventions every agent follows live in `docs/pr-review/review-conventions.md` (severity, criterion
markers, reporting discipline) and `docs/pr-review/comment-style.md` (comment wording).
...
```bash
PR_NUMBER=<PR_NUMBER>
mkdir -p /tmp/toolkit-pr-review-${PR_NUMBER}/files
gh pr view ${PR_NUMBER} --repo ${REPO} \
  --json number,title,body,headRefOid,baseRefOid,baseRefName,headRefName \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/meta.json
gh pr diff ${PR_NUMBER} --repo ${REPO} \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/diff.patch
echo "HEAD SHA: $(jq -r .headRefOid /tmp/toolkit-pr-review-${PR_NUMBER}/meta.json)"
```

## Step 3: Parse diff — identify files, ranges, classify ToolKit-owned

Parse `/tmp/toolkit-pr-review-${PR_NUMBER}/diff.patch` to extract:
...
cat > /tmp/toolkit-pr-review-${PR_NUMBER}/context.json << 'EOF'
{
  "pr_number": <PR_NUMBER>,
  "repo": "<REPO>",
  "head_sha": "<HEAD_SHA>",
...
#   | base64 -d > /tmp/toolkit-pr-review-${PR_NUMBER}/files/path__to__file.rs
...
#     | base64 -d > /tmp/toolkit-pr-review-${PR_NUMBER}/files/deny.toml 2>/dev/null || true
...
# BASE_SHA=$(jq -r .baseRefOid /tmp/toolkit-pr-review-${PR_NUMBER}/meta.json)
...
Spawn all applicable agents as parallel background processes. Each reads its inputs from `/tmp/toolkit-pr-review-${PR_NUMBER}/` and writes a JSON array to its output file.

Skip Agent E if `toolkit_owned_files` is empty. Agent F is **not** gated: it runs whenever
`rust_files` is non-empty, because a PR that adds production code and no tests is exactly what
`RUST-TEST-001` exists to catch.
...
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-errors.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-errors.json 2>&1 &

# Agent B — Security
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-security.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-security.json 2>&1 &

# Agent C — Async, Concurrency, Performance
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-async.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-async.json 2>&1 &

# Agent D — Design, Types, Architecture
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-design.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-design.json 2>&1 &

# Agent E — ToolKit Framework Compliance (gated)
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-toolkit.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-toolkit.json 2>&1 &

# Agent F — Test Quality
claude -p "$(sed '/^---$/,/^---$/d;1{/^---/d}' ${AGENTS_DIR}/toolkit-pr-review-tests.md)

The PR number is ${PR_NUMBER}. Read /tmp/toolkit-pr-review-${PR_NUMBER}/context.json first." \
  > /tmp/toolkit-pr-review-${PR_NUMBER}/out-tests.json 2>&1 &
...
# Second pass: two agents reaching the same line from different rules are describing
# the same defect from two angles. Posting both puts two comments on one line, so keep
# only the highest-severity one. Deleted files have no line and are left alone.
...
```bash
export PR_NUMBER=<PR_NUMBER>
export HEAD_SHA=$(jq -r .headRefOid /tmp/toolkit-pr-review-${PR_NUMBER}/meta.json)
